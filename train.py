...
def get_data_loaders(args, tokenizer):
    ...
    pchat = get_dataset(tokenizer, args.dataset_path, args.dataset_cache)
    
    assert sorted(list(pchat.keys())) == ['train', 'valid']
    
    for dataset_name in ['train', 'valid']:
        dataset = pchat[dataset_name]
        
        # Number of candidates for MC task
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        lm_labels = bool(j == num_candidates - 1) # !! Is last candidate always correct?
...
if __name__ == "__main__":
    ...
    if args.local_rank in [-1, 0]:
        ...
        tb_logger = TensorboardLogger(logdir=None)
